...
class DeliveryManDisbursementController extends Controller
{
        ...
        foreach ($disbursements as $disbursement){
            ...
            if ( (string)  $disbursement->delivery_man?->wallet?->total_earning <  (string) ($disbursement->delivery_man?->wallet?->total_withdrawn + $disbursement->delivery_man?->wallet?->pending_withdraw) ) {
                return response()->json([
                    'status' => 'error',
                    'message'=> translate('messages.Blalnce_mismatched_total_earning_is_too_low_for').' '.$disbursement->delivery_man?->name,
                ]);
            }
            ...
            if($request->status == 'completed') {
                ...
                if($disbursement->status == 'completed'){
                    $withdraw = ProvideDMEarning::where('ref',$disbursement->id)->where('delivery_man_id', $disbursement->delivery_man_id)->first();
                    if ($withdraw){
                        $withdraw->delete();
                    }
                }
    ...
    public function statusById($id,$status)
    {
        ...
        if ((string) $disbursement->delivery_man?->wallet?->total_earning <  (string)($disbursement->delivery_man?->wallet?->total_withdrawn + $disbursement->delivery_man?->wallet?->pending_withdraw) ) {
                ...
                Toastr::error(translate('messages.Blalnce_mismatched_total_earning_is_too_low_for').' '.$disbursement->delivery_man?->f_name);
                return back();
            }
