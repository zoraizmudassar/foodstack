...
// dd($header_app_button_status);
    ...
    public function availableZone(){
        ...
        return view('admin-views.landing_page.available-zone' ,compact('language', 'available_zone_title','available_zone_short_description' ,'available_zone_image','available_zone_status'));
    }
    public function reactAvailableZone(){

        $language = getWebConfig('language');
        $available_zone_title=DataSetting::withoutGlobalScope('translate')->where('type','react_landing_page')->where('key','available_zone_title')->first();
        $available_zone_short_description=DataSetting::withoutGlobalScope('translate')->where('type','react_landing_page')->where('key','available_zone_short_description')->first();
        $available_zone_image=DataSetting::withoutGlobalScope('translate')->where('type','react_landing_page')->where('key','available_zone_image')->first();
        $available_zone_status=DataSetting::withoutGlobalScope('translate')->where('type','react_landing_page')->where('key','available_zone_status')->first()?->value ?? 0;

        return view('admin-views.landing_page.react.available-zone' ,compact('language', 'available_zone_title','available_zone_short_description' ,'available_zone_image','available_zone_status'));
    }
    public function availableZoneUpdate(Request $request){
        ...
        DataSetting::updateOrInsert(['type' => $page_type,'key' => 'available_zone_status'], [
